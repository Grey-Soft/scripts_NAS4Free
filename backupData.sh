...
ensureRemoteFSExists() {
	...
	[ "$readonly_mode" = "on" ] && if ! $RUN_CMD_SSH $BIN_ZFS set readonly=off $pool >/dev/null; then
		log_error "$LOGFILE" "The destination pool \"$pool\" could not be set to READONLY=off. Filesystem creation not possible"
		return 1
	fi
	...
	if ! $RUN_CMD_SSH $BIN_ZFS create -p "$fs"; then
		...
		[ "$readonly_mode" = "on" ] && if ! $RUN_CMD_SSH $BIN_ZFS set readonly=on $pool >/dev/null; then
			log_error "$LOGFILE" "The destination pool \"$pool\" could not be set to \"readonly=on\""
		fi
		...
		if ! $RUN_CMD_SSH $BIN_ZFS set readonly=on $fs >/dev/null; then
			log_error "$LOGFILE" "The destination filesystem \"$fs\" could not be set to \"readonly=on\""
		fi

		# Set the destination pool to readonly
		[ "$readonly_mode" = "on" ] && if ! $RUN_CMD_SSH $BIN_ZFS set readonly=on $pool >/dev/null; then
			log_warning "$LOGFILE" "The destination pool \"$pool\" could not be set to \"readonly=on\""
		fi
